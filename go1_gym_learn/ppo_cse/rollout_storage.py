...
class RolloutStorage:
    ...
    def _save_hidden_states(self, hidden_states):
        ...
        # hid shape:[hid/cell, num_layers, num_envs, h_dim]
        if isinstance(hidden_states[0], (tuple, list)):  # lstm or transformer-XL
            hid_a = hidden_states[0]
        else:
            hid_a = (hidden_states[0],)  # GRU or TCN
        ...
        # copy the states
        for i in range(len(hid_a)):  # 每层RNN最后一时刻的隐藏状态
    ...
    def reccurent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                if self.saved_hidden_states_a is not None:
                    # 此处for循环主要是因为(lstm和gru的区别，隐藏状态种类不一样)
                    hid_a_batch = [
                        saved_hidden_states.permute(2, 0, 1, 3)[last_was_done][first_traj:last_traj].transpose(1,
                                                                                                               0).contiguous()
                        for saved_hidden_states in self.saved_hidden_states_a]
                    # remove the tuple for GRU
                    hid_a_batch = hid_a_batch[0] if len(
                        hid_a_batch) == 1 else hid_a_batch
                else:
                    hid_a_batch = None
                ...
                if self.saved_hidden_states_c is not None:
                    ...
                    hid_c_batch = hid_c_batch[0] if len(
                        hid_c_batch) == 1 else hid_c_batch
                else:
                    hid_c_batch = None
