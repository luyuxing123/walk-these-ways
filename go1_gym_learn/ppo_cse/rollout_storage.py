...
class RolloutStorage:
    ...
    def add_transitions(self, transition: Transition):
        ...
        self._save_amp_transitions(transition.amp_trans)
        self.step += 1

    def clear(self):
        self.step = 0

    def _save_amp_transitions(self, amp_trans):
        if amp_trans is None:
            return
        if self.amp_trans is None:
            self.amp_trans = torch.zeros(self.observations.shape[0], *amp_trans.shape, device=self.device)
        self.amp_trans[self.step].copy_(amp_trans)
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        amp_trans = self.amp_trans.flatten(0, 1)

        for epoch in range(num_epochs):
            self.first_epoch = not epoch
            for i in range(num_mini_batches):
                ...
                amp_trans_batch = amp_trans[batch_idx]
                ...
                yield obs_batch, critic_observations_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, \
                       old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, None, env_bins_batch,amp_trans_batch
