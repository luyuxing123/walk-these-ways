        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    #前2048个用于平地，后2048个用于各种地形，使用自适应步态
                    actions_normal = self.alg.act(obs[:2048], privileged_obs[:2048],
                                                 obs_history[:2048],is_normal=True)
                    action_adaptative = self.alg.act(obs[2048:], privileged_obs[2048:],
                                                 obs_history[2048:],is_normal=False)
                    actions_train = action_adaptative
                    # actions_train = self.alg.act(obs[:num_train_envs], privileged_obs[:num_train_envs],
                    #                              obs_history[:num_train_envs])
                    actions_eval=  torch.empty(0, 12,device=self.device)

                    if num_train_envs != self.env.num_train_envs:
                        if eval_expert:
