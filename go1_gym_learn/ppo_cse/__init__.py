...
class Runner:
    ...
    def __init__(self, env, device='cpu'):
        ...
        amp_normalizer = Normalizer(self.env.num_obs)
        ...
        self.alg = PPO(actor_critic,discriminator, amp_normalizer,device=self.device)
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    amp_rewards = self.alg.discriminator.predict_amp_reward(
                        obs[2000:,:30], obs[2000:,30:], normalizer=self.alg.amp_normalizer)[0]
                    # last_obs = obs
                    rewards[2000:] += amp_rewards
