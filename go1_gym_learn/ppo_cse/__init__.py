...
class RunnerArgs(PrefixProto, cli=False):
    ...
    MOTION_FILES = glob.glob('/home/ask-spc/luyx/walk-these-ways-asymmetric/datasets/go1/*')
...
class Runner:
    ...
    def __init__(self, env, device='cpu'):
        ...
        amp_data = AMPLoader(
            ...
            motion_files=RunnerArgs.MOTION_FILES)
        ...
        self.alg = PPO(actor_critic, discriminator, amp_data, amp_normalizer, device=self.device)
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False, eval_freq=100, curriculum_dump_freq=500, eval_expert=False):
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    obs, privileged_obs, obs_history = obs_dict["obs"], obs_dict["privileged_obs"], obs_dict[
                        "obs_history"]
                    obs, privileged_obs, obs_history, rewards, dones,next_amp_obs = obs.to(self.device), privileged_obs.to(
                        self.device), obs_history.to(self.device), rewards.to(self.device), dones.to(self.device),next_amp_obs.to(self.device)

                    #todo:终止时刻状态特殊处理.
                    #todo：inverse仅从过去两次state推导出action似乎有点困难。使用obs history推导
                    # if i==0:
                    #     last_obs = obs
                    #     last_pri_obs = privileged_obs
                    # elif i==1:
                    #     current_obs = obs
                    #     current_pri_obs = privileged_obs
                    # else:
                    #     # forward_reward,inverse_reward,for_loss,inv_loss = self.alg.actor_critic.compute_intrinsic_reward(current_obs, current_pri_obs,
                    #     #                                                                   last_obs, last_pri_obs,
                    #     #                                                                   actions, obs)
                    #     forward_reward,inverse_reward,for_loss,inv_loss = self.alg.actor_critic.compute_intrinsic_reward(obs_history,actions, obs)
                    #
                    #
                    #     forward_reward *= 1e-6
                    #     inverse_reward *= 1e-6
                    #     # rewards += forward_reward+inverse_reward
                    ...
                    #     last_pri_obs = current_pri_obs
                    #     current_obs = obs
                    #     current_pri_obs = privileged_obs


                    # Account for terminal states.
                    #采取一个action后，环境变为下一个state，因此amp obs也会变成下一个obs
                    #但是部分环境可能提前终止，所以在step之后调用get_amp_observations获取的是终止之后新的环境
                    #terminal_amp_states存储的是重置环境之前的state，需要单独赋值
                    next_amp_obs_with_term = torch.clone(next_amp_obs)
                    next_amp_obs_with_term[reset_env_ids] = terminal_amp_states

                    #先获取任务奖励，然后将任务奖励传进这个函数，计算并加上discriminator的奖励返回
                    # amp_rewards = self.alg.discriminator.predict_amp_reward(
                    #     amp_obs, next_amp_obs_with_term, normalizer=self.alg.amp_normalizer)[0]
                    # rewards += amp_rewards

                    self.alg.process_env_step(rewards, dones, infos, amp_obs,next_amp_obs_with_term)
                    #注意此处用的是next_amp_obs，而不含终止状态，因为下一个状态是从新的状态开始的
            ...
            mean_value_loss, mean_surrogate_loss, mean_adaptation_module_loss, mean_decoder_loss, mean_decoder_loss_student, mean_adaptation_module_test_loss, mean_decoder_test_loss, mean_decoder_test_loss_student ,mean_amp_loss= self.alg.update()
            stop = time.time()
            learn_time = stop - start

            log_forward_loss = forward_loss/self.num_steps_per_env
            log_inverse_loss = inverse_loss/self.num_steps_per_env
            ...
            logger.store_metrics(
                ...
                forward_loss=log_forward_loss,
                inverse_loss=log_inverse_loss
