...
class PPO:
    ...
    def __init__(self, actor_critic, discriminator, amp_data, amp_normalizer,device='cpu'):
        ...
        self.amp_storage = ReplayBuffer(
            discriminator.input_dim // 2, 1000000, device)
    ...
    def act(self, obs, privileged_obs, obs_history):
        ...
        self.transition.actions = self.actor_critic.act(obs_history,privileged_obs).detach()
    ...
    def process_env_step(self, rewards, dones, infos, amp_obs_current,amp_obs_next):
        ...
        self.transition.amp_trans = torch.cat((amp_obs_current, amp_obs_next), dim=-1)
    ...
    def update(self):
        ...
        amp_expert_generator = self.amp_data.feed_forward_generator(
            PPO_Args.num_learning_epochs * PPO_Args.num_mini_batches,
            self.storage.num_envs * self.storage.num_transitions_per_env //
                PPO_Args.num_mini_batches)

        for  sample,sample_amp_expert in zip(generator, amp_expert_generator):
            obs_batch, critic_obs_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
                old_mu_batch, old_sigma_batch, masks_batch, env_bins_batch,amp_trans_batch = sample
            self.actor_critic.act(obs_history_batch,privileged_obs_batch, masks=masks_batch)
            ...
            # Discriminator loss.
            # expert_transition = torch.cat(sample_amp_expert, dim=-1)
            # norm_amp_obs = self.discriminator.norm_obs(torch.cat(
            #     [amp_trans_batch , expert_transition], dim=0), train=self.storage.first_epoch)
            # d_score = self.discriminator(norm_amp_obs)
            # policy_d = d_score[:norm_amp_obs.shape[0]//2]
            # expert_d = d_score[norm_amp_obs.shape[0]//2:]
            #
            ...
            for epoch in range(PPO_Args.num_adaptation_module_substeps):
                # ht = self.actor_critic.history_encoder(obs_history_batch)
        ...
        mean_amp_loss /= (num_updates * PPO_Args.num_adaptation_module_substeps)
        self.storage.clear()

        return mean_value_loss, mean_surrogate_loss, mean_adaptation_module_loss, mean_decoder_loss, mean_decoder_loss_student, mean_adaptation_module_test_loss, mean_decoder_test_loss, mean_decoder_test_loss_student,mean_amp_loss
