...
class RunningStandardScaler(nn.Module):
    ...
    def _compute(self, x: torch.Tensor, train: bool = False, inverse: bool = False, update_only: bool = False) -> torch.Tensor:
        ...
        if train:
            if x.dim() == 3:
                self._parallel_variance(torch.mean(x, dim=(0, 1)), torch.var(
                    x, dim=(0, 1)), x.shape[0] * x.shape[1])
            else:
                self._parallel_variance(torch.mean(
                    x, dim=0), torch.var(x, dim=0), x.shape[0])
            if update_only:
                return

        # scale back the data to the original representation
        if inverse:
            return torch.sqrt(self.running_variance[:x.shape[-1]].float()) \
                * torch.clamp(x, min=-self.clip_threshold, max=self.clip_threshold) + self.running_mean[:x.shape[-1]].float()
        # standardization by centering and scaling
        else:
            return torch.clamp((x - self.running_mean[:x.shape[-1]].float()) / (torch.sqrt(self.running_variance[:x.shape[-1]].float()) + self.epsilon),
                               min=-self.clip_threshold, max=self.clip_threshold)

    def forward(self, x: torch.Tensor, train: bool = False, inverse: bool = False, no_grad: bool = True, update_only: bool = False) -> torch.Tensor:
        ...
        if no_grad:
            with torch.no_grad():
                return self._compute(x, train, inverse, update_only)
        else:
            return self._compute(x, train, inverse, update_only)
...
class AMPDiscriminator(nn.Module):
    ...
    def predict_amp_reward(
            self, state, action,  normalizer=None):
        with torch.no_grad():
            self.eval()
            norm_amp_obs = self.norm_obs(torch.cat([state, action], dim=-1))
