...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: Cfg, sim_params, physics_engine, sim_device, headless, eval_cfg=None,
        ...
        # 创建一个发布者，话题类型为geometry_msgs/WrenchStamped
        self.force_pub = rospy.Publisher('/contact_forces_FL', WrenchStamped, queue_size=10)

    def publish_forces(self):
        # 假设self.contact_forces[:, self.feet_indices, :] 已经计算好，且为torch张量
        ...
        # 计算合力（欧几里得范数）
        force_vector = contact_forces[0, 0, :]  # 取出 [force_x, force_y, force_z]
        total_force = torch.norm(force_vector)  # 计算合力
        # 将合力应用到z方向，仅作示例（你可以根据需求修改）
        force_msg.wrench.force.z = total_force.item()
        # 填充消息的时间戳
        force_msg.header.stamp = rospy.Time.now()
        # force_msg.header.frame_id = f'foot_{i}'  # 为不同的脚设置不同的frame id
        # 发布消息
    ...
    def step(self, actions):
        ...
        # print(self.contact_forces[:, self.env.feet_indices, :])
        # self.publish_forces()
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        self.terrain_levels[env_ids] = torch.where(self.terrain_levels[env_ids] >= self.cfg.terrain.center_span*2,
    ...
    def _resample_commands(self, env_ids):
        ...
        #
        # #add by luyx,only x,y veolicity,no yaw speed command。only forward command
        # self.commands[env_ids, 2] = 0
        # self.commands[:, 0] = torch.abs(self.commands[:, 0])

        self.v_level[env_ids] = torch.clip(
            1 * torch.norm(self.commands[env_ids, :2], dim=-1) + 0.5 * torch.abs(self.commands[env_ids, 2]), min=1)
        ...
        if True or self.cfg.env.observe_gait_commands and self.cfg.commands.num_commands>3:
            if self.cfg.env.observe_gait_commands:
                if self.cfg.commands.num_commands > 3:
                    ...
                    frequencies = 1.5 #self.cfg.commands.limit_gait_frequency[0]
                    phases = 0.5  # self.commands[:, 5]
                    offsets = 0  # self.commands[:, 6]
                    bounds = 0  # self.commands[:, 7]
                    durations = 0.5  # self.commands[:, 8]
