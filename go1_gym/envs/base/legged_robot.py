...
class LeggedRobot(BaseTask):
    ...
    def compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            ...
            # self.rew_buf += rew
            if name in ['tracking_contacts_shaped_vel', 'tracking_contacts_shaped_force',
                         'raibert_heuristic','base_height']:
                self.rew_buf[:self.num_envs // 2] += rew[:self.num_envs // 2]
            else:
                self.rew_buf += rew
    ...
    def compute_observations(self):
        ...
        self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                             ...
                                             self.contact_forces[:, self.feet_indices, :].reshape(self.cfg.env.num_envs,
                                                                                                  -1) * 0.01,
                                             unscale_np(self.friction_coeffs[:, 0].unsqueeze(1), friction_range[0],
                                                        friction_range[1]),
                                             self.collision_states,
                                             ), dim=1)
        command = torch.cat((self.commands,self.clock_inputs),dim=-1)
        command[:,0] *= 2
        command[:,1] *= 2
        command[:,2] *= 0.25
        command[:,3] *= 2
    ...
    def _create_envs(self):
        ...
        if self.cfg.env.random_robot_size:
            ...
            for i in range(self.num_envs//2,self.num_envs):
                if i%100 == 0:
                    scale = 0.8+(i-self.num_envs//2)/100*0.02
                    if self.cfg.env.run_play:
                        scale = self.cfg.env.play_robot_scale
                    ...
                    for origin_elem in root.iter('origin'):
                        ...
                        if xyz:
                            ...
                            if len(xyz_values) == 3:
                                ...
                                if x == 0.1881 or x == -0.1881:
                                    x *= scale
                                    origin_elem.set('xyz', f'{x} {y} {z}')

                    for origin_elem in root.iter('limit'):
                        lower = float(origin_elem.get('lower'))
                        upper = float(origin_elem.get('upper'))
                        effort = float(origin_elem.get('effort'))
                        length = upper - lower
                        length *= scale
                        lower_new = (lower+upper)/2 - length/2
                        upper_new = (lower+upper)/2 + length/2
                        effort_new = effort*scale
                        origin_elem.set('lower', str(lower_new))
                        origin_elem.set('upper', str(upper_new))
                        origin_elem.set('effort', str(effort_new))
