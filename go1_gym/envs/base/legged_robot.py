...
from go1_gym_learn.datasets.motion_loader import AMPLoader
from go1_gym.utils.math_utils import class_to_dict
...
class LeggedRobot(BaseTask):
    ...
    def post_physics_step(self):
        ...
        self.foot_vel_world = self.foot_velocities
        ...
        #在每一个时间步检查是否走出该地形，如果走出则该agent可以上升一个难度级别，而不是reset时才检查。add by luyx
        distance = torch.norm(self.root_states[:, :2] - self.env_origins[:, :2], dim=1)
        move_up = distance > self.cfg.terrain.terrain_length / 2
        self.move_up = torch.logical_or(self.move_up,move_up)
        # self.move_down = (distance < torch.norm(self.commands[:, :2],
        #                                    dim=1) * self.cfg.env.episode_length_s * 0.5) * ~self.move_up
    ...
    def check_termination(self):
        ...
        if self.cfg.rewards.use_terminal_body_height:
            #root_states:4096*13,measured:4096*187.root_states取身体高度，去掉最后一维，变为4096*1
            #对于这4096个agent，每个高度与187个高度相减然后得到的值取平均值，即为相对身体高度
    ...
    #     #from legged gym,terrain curriculum
    ...
    #         return
    #     self.terrain_levels[env_ids] += 1 * self.move_up[env_ids]   #- 1 * self.move_down[env_ids]
    #     # Robots that solve the last level are sent to a random one
    #     self.terrain_levels[env_ids] = torch.where(self.terrain_levels[env_ids] >= self.cfg.terrain.num_rows-1,#self.cfg.terrain.center_span*2,
    ...
    #
    #     #更新完地形课程后，将对应环境的标志位置false
    #     self.move_up[env_ids] = False
    #     # self.move_down[env_ids] = False
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        des_distance = torch.norm(
            self.commands[env_ids, :2], dim=1) * self.max_episode_length_s

        # is_success = True
        # for key, value in self.curriculum_thresholds['terrains'].items():
        ...
        #     is_success = is_success * (task_reward > success_threshold)

        # robots that walked far enough progress to harder terains
        self.terrain_levels[env_ids] -= 1*(real_distance < des_distance * 0.4)
        # self.terrain_levels[env_ids] += 1 * (real_distance > self.cfg.terrain.terrain_length * 0.4) * ~self.large_ori_buf[env_ids] #* is_success
        self.terrain_levels[env_ids] += 1 * self.move_up[env_ids]
        self.terrain_levels[env_ids] = torch.where(self.terrain_levels[env_ids] >= self.cfg.terrain.num_rows-1,#self.cfg.terrain.center_span*2,
                                                   torch.randint_like(self.terrain_levels[env_ids],
                                                                      self.cfg.terrain.num_rows),
                                                   torch.clip(self.terrain_levels[env_ids],
                                                              1))  # (the minumum level is zero)
        torch.clip(self.terrain_levels[env_ids],min=2)
    ...
    def compute_observations(self):
        ...
        if self.cfg.env.observe_vel:
            if self.cfg.commands.global_reference:
                self.obs_buf = torch.cat((self.obs_buf,self.root_states[:self.num_envs, 7:10] * self.obs_scales.lin_vel
                                          ), dim=-1)
            else:
                self.obs_buf = torch.cat((self.obs_buf,self.base_lin_vel * self.obs_scales.lin_vel
        ...
        if self.cfg.env.observe_foot_forces:
            self.obs_buf = torch.cat((self.obs_buf,
                                             self.contact_forces[:, self.feet_indices, :].reshape(self.cfg.env.num_envs, -1)*0.01),
                                             dim=-1)
        if self.cfg.env.observe_friction:
            friction_range = self.cfg.domain_rand.friction_range
            self.obs_buf = torch.cat((self.obs_buf,
                                      unscale_np(self.friction_coeffs[:, 0].unsqueeze(1), friction_range[0], friction_range[1])),
                                      dim=-1)

        if self.cfg.env.observe_collision_state:
            self.obs_buf = torch.cat((self.obs_buf,self.collision_states),dim=-1)

        if self.cfg.env.observe_stair_height_width:
            stair_height = torch.zeros(self.num_envs,device=self.obs_buf.device,requires_grad=False)
            stair_width = torch.zeros(self.num_envs,device=self.obs_buf.device,requires_grad=False)
            for i in range(self.num_envs):
                row = ((self.env_origins[i, 0] -4) // 8 ).long()
                col = ((self.env_origins[i, 1] -4) // 8 ).long()
                stair_height[i] = self.cfg.terrain.stair_height[row][col]
                stair_width[i] = self.cfg.terrain.stair_width[row][col]
            self.obs_buf = torch.cat((self.obs_buf,stair_height.unsqueeze(-1),stair_width.unsqueeze(-1)),dim=-1)

        #先本体感知，然后特权信息，最后地形信息（58，24，187）
        if self.cfg.env.observe_measure_heights:
            heights = torch.clip(self.root_states[:, 2].unsqueeze(1) - self.measured_heights
                                 - self.cfg.rewards.base_height_target, -1, 1.)
        ...
        if self.cfg.env.priv_observe_foot_forces:   #added by luyx 每只脚3个力，共四个
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                                 self.contact_forces[:, self.feet_indices, :].reshape(self.cfg.env.num_envs, -1)*0.01),dim=-1)
        ...
        if self.cfg.env.priv_observe_collision_state:
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf, self.collision_states), dim=-1)

        if self.cfg.env.priv_observe_stair_height_width:
            stair_height = torch.zeros(self.num_envs,device=self.obs_buf.device,requires_grad=False)
            stair_width = torch.zeros(self.num_envs,device=self.obs_buf.device,requires_grad=False)
            for i in range(self.num_envs):
                row = ((self.env_origins[i, 0] -4) // 8 ).long()
                ...
                stair_height[i] = self.cfg.terrain.stair_height[row][col]
                stair_width[i] = self.cfg.terrain.stair_width[row][col]
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,stair_height.unsqueeze(-1),stair_width.unsqueeze(-1)),dim=-1)
        ...
        command = torch.cat((self.clock_inputs,self.commands[:,3:9],self.commands[:,0:3]), dim=-1)
        command_scale = torch.tensor([1,1,1,1, 2,1,1,1,1,1, 2,2,0.25],device=command.device)
        command = command*command_scale

        #为了方便起见，将速度命令和步态信息放到特权信息的最后
        self.privileged_obs_buf = torch.cat((self.privileged_obs_buf, command), dim =-1)
    ...
    def get_amp_observations(self):
        ...
        return torch.cat((base_lin_vel, base_ang_vel,joint_pos,  joint_vel), dim=-1)


    # def _resample_commands(self, env_ids):
    #     x_vel = (torch.rand(self.commands[env_ids, 0].shape, dtype=torch.float, device=self.device,
    #                                     requires_grad=False) - 0.5) * 1.2
    #     y_vel = (torch.rand(self.commands[env_ids, 1].shape, dtype=torch.float, device=self.device,
    #                                     requires_grad=False) - 0.5) * 0.6
    #     yaw_vel = (torch.rand(self.commands[env_ids, 2].shape, dtype=torch.float, device=self.device,
    #                                     requires_grad=False) - 0.5) * 1.2
    #
    #     rand = random.randint(3,6)
    #     if rand == 3:
    #         self.commands[env_ids, 0] = random.randint(2,6)/10
    #         self.commands[env_ids, 1] = 0
    #         self.commands[env_ids, 2] = 0
    #     elif rand == 4:
    #         self.commands[env_ids, 0] = 0
    #         self.commands[env_ids, 1] = y_vel
    #         self.commands[env_ids, 2] = 0
    #     elif rand == 5:
    #         self.commands[env_ids, 0] = 0
    #         self.commands[env_ids, 1] = 0
    #         self.commands[env_ids, 2] = yaw_vel
    #     else:
    #         self.commands[env_ids, 0] = x_vel
    #         self.commands[env_ids, 1] = y_vel
    #         self.commands[env_ids, 2] = yaw_vel
    #
    #     self.commands[env_ids, :2] *= (torch.norm(self.commands[env_ids, :2], dim=1) > 0.2).unsqueeze(1)
    #
    #     self.commands[env_ids, 3] = random.uniform(0.26, 0.3)
    #     self.commands[env_ids, 4] = random.uniform(1.5,2)
    #     # self.commands[env_ids, 4] = random.uniform(2,4)
    #     # self.commands[env_ids, 8] = random.randint(25,75)/100
    #     self.commands[env_ids, 8] = 0.5
    #
    #     random_category = random.choice(self.category_names)
    #     if random_category == 'pronk':
    #         self.commands[env_ids, 5] = 0.0
    #         self.commands[env_ids, 6] = 0.0
    #         self.commands[env_ids, 7] = 0.0
    #     elif random_category == 'trot':
    #         self.commands[env_ids, 5] = 0.5
    #         self.commands[env_ids, 6] = 0.0
    #         self.commands[env_ids, 7] = 0.0
    #     elif random_category == 'pace':
    #         self.commands[env_ids, 5] = 0.0
    #         self.commands[env_ids, 6] = 0.0
    #         self.commands[env_ids, 7] = 0.5
    #     elif random_category == 'bounding':
    #         self.commands[env_ids, 5] = 0.0
    #         self.commands[env_ids, 6] = 0.5
    #         self.commands[env_ids, 7] = 0.0
    #     # elif random_category == 'walk':
    #     #     self.commands[env_ids, 5] = 0.5
    #     #     self.commands[env_ids, 6] = 0.25
    #     #     self.commands[env_ids, 7] = 0.75
    ...
    def _resample_commands(self, env_ids):
            ...
            if self.cfg.commands.gaitwise_curricula:
                for i, (category, env_ids_in_category) in enumerate(zip(self.category_names, category_env_ids)):
                    if category == "pronk":  # pronking
                        self.commands[env_ids_in_category, 5] = 0.5
                        self.commands[env_ids_in_category, 6] = 0.5
                        self.commands[env_ids_in_category, 7] = 0.5
        ...
        # flat_env = torch.where(self.env_origins[:,1]>36)[0]
        # torch.clip(self.commands[flat_env,:3],min=-0.6,max=0.6)
        # torch.clip(self.commands[flat_env,3], min=0.28, max=0.28)
    ...
    def _step_contact_targets(self):
        if True or self.cfg.env.observe_gait_commands and self.cfg.commands.num_commands>3:
            ...
            # contact_mask = torch.norm(self.commands, dim=-1) <= 0.1
            # self.desired_contact_states[contact_mask, :] = 1
            # if (~contact_mask).sum() > 0:
            #     self.desired_contact_states[~contact_mask, 0] = smoothing_multiplier_FL[~contact_mask]
            #     self.desired_contact_states[~contact_mask, 1] = smoothing_multiplier_FR[~contact_mask]
            #     self.desired_contact_states[~contact_mask, 2] = smoothing_multiplier_RL[~contact_mask]
            #     self.desired_contact_states[~contact_mask, 3] = smoothing_multiplier_RR[~contact_mask]
    ...
    def _teleport_robots(self, env_ids, cfg):
        ...
        if cfg.terrain.teleport_robots:
            ...
            self.root_states[low_x_ids, 2] += 1
            self.root_states[high_x_ids, 2] += 1
            self.root_states[low_y_ids, 2] += 1
            self.root_states[low_y_ids, 2] += 1
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.env.observe_friction:
            noise_vec = torch.cat((noise_vec,torch.zeros(4)),dim=-1)

        if self.cfg.env.observe_collision_state:
            noise_vec = torch.cat((noise_vec,torch.zeros(8)),dim=-1)
    ...
    def _init_buffers(self):
        ...
        self.curriculum_reward_list = [
            "dof_vel_limits", "torque_limits",
            "action_rate", "dof_vel", "dof_acc", "torques",
            "lin_vel_z", "ang_vel_xy",
            "collision", "trap_static",
            "tracking_ang_vel", "foot_stance_ori",
        ]
        self.large_ori_buf = torch.zeros(self.num_envs, dtype=torch.bool,device=self.device,requires_grad=False)
        self.no_contact_count = torch.zeros(self.num_envs, dtype=torch.int,device=self.device,requires_grad=False)
        ...
        self.move_up = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device, requires_grad=False)
        # self.move_down = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device, requires_grad=False)

        # create some wrapper tensors for different slices
        self.v_level = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.bool_foot_contact = torch.zeros(self.num_envs, self.feet_indices.shape[0], dtype=torch.bool, device=self.device, requires_grad=False)
        ...
        self.foot_positions = self.rigid_body_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_indices,
                              0:3]
        self.last_foot_positions = self.rigid_body_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_indices,
                              0:3]
        self.prev_base_pos = self.base_pos.clone()
        self.prev_foot_velocities = self.foot_velocities.clone()
        ...
        self.collision_states = torch.zeros(
            self.num_envs, self.penalised_contact_indices.shape[0], dtype=torch.bool, device=self.device,
            requires_grad=False)
    ...
    def _create_envs(self):
        ...
        if self.cfg.env.random_robot_size:
            for i in range(self.num_envs):
                if i%400 == 0:
                    scale = 0.7+i/400*0.1
                    scale = 1
                    ...
                    for mass_elem in root.iter('mass'):
                        original_mass = float(mass_elem.get('value', 1.0))
                    ...
                    for box_elem in root.iter('box'):
                        ...
                        if size_str:
                            sizes = list(map(float, size_str.split()))
                            new_length = sizes[0] * scale
                            new_width = sizes[1] * scale
                            new_height = sizes[2] * scale
                            new_sizes = [new_length, new_width, new_height]
                            box_elem.set('size', ' '.join(map(str, new_sizes)))

                    for origin_elem in root.iter('origin'):
                        xyz = origin_elem.get('xyz')
                        if xyz:
                            xyz_values = xyz.split()
                            if len(xyz_values) == 3:
                                x, y, z = map(float, xyz_values)
                                if z == -0.213 or z == -0.1065:  # 检查 z 是否是这两个特定的值
                                    # 更新 z 坐标，乘以对应的 `scale_factor_length`
                                    z *= scale
                                    origin_elem.set('xyz', f'{x} {y} {z}')

                    modified_urdf_content = ET.tostring(root, encoding='utf-8', method='xml').decode()
                    # 将修改后的 URDF 内容保存为临时文件
                    temp_urdf_path = os.path.join(os.path.dirname(asset_path), 'go1_temp.urdf')
                    with open(temp_urdf_path, 'w') as f:
                        f.write(modified_urdf_content)
                    self.robot_asset = self.gym.load_asset(self.sim, os.path.dirname(temp_urdf_path), os.path.basename(temp_urdf_path), asset_options)
    ...
    def _get_env_origins(self, env_ids, cfg):
        ...
        if cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            self.custom_origins = True
            min_terrain_level = 1 #cfg.terrain.num_rows // 2 - cfg.terrain.center_span
            max_terrain_level = cfg.terrain.num_rows -1  #// 2 + cfg.terrain.center_span - 1
            min_terrain_type = cfg.terrain.num_cols // 2 - cfg.terrain.center_span
            max_terrain_type = cfg.terrain.num_cols // 2 + cfg.terrain.center_span - 1
            self.env_origins = torch.zeros(self.num_envs, 3, device=self.device, requires_grad=False)
            # put robots at the origins defined by the terrain
            max_init_level = min_terrain_level
            if not self.cfg.terrain.curriculum: max_init_level = self.cfg.terrain.num_rows - 1
            # self.terrain_levels[env_ids] = torch.randint(min_terrain_level, max_init_level + 1, (len(env_ids),),device=self.device)
            self.terrain_levels[env_ids] = torch.randint(min_terrain_level, max_init_level + 1 , (len(env_ids),),device=self.device)
            self.terrain_types[env_ids] = torch.randint(min_terrain_type, max_terrain_type + 1, (len(env_ids),),device=self.device)
            self.max_terrain_level = self.cfg.terrain.num_rows
            self.terrain_origins = torch.from_numpy(self.cfg.terrain.env_origins).to(self.device).to(torch.float)
