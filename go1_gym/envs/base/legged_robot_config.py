...
class Cfg(PrefixProto, cli=False):
    class env(PrefixProto, cli=False):
        amp_motion_files=glob.glob('/home/ask-spc/luyx/walk-these-ways-asymmetric/datasets/go1/*')
        reference_state_initialization = False
        random_robot_size = True
        use_teacher = False
        num_envs = 4000
        num_observations = 42   #6+12*3+13(command,9+4)
        # if not None a privilige_obs_buf will be returned by step() (critic obs for assymetric training). None is returned otherwise
        num_privileged_obs = 211+13
        ...
        #teacher
        observe_measure_heights = False
        observe_foot_forces = False
        observe_vel = False
        observe_friction = False
        observe_collision_state =False
        observe_stair_height_width = False
    ...
    class terrain(PrefixProto, cli=False):
        ...
        terrain_proportions = [0,1,0.875,1]
        # trimesh only:
