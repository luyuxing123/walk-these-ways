...
class Cfg(PrefixProto, cli=False):
    class env(PrefixProto, cli=False):
        ...
        priv_observe_foot_forces = True
        priv_observe_vel = True
        priv_observe_friction = True
        priv_observe_collision_state =True
        priv_observe_stair_height_width = True

        #teacher
        observe_measure_heights = True
        observe_foot_forces = True
        observe_vel = True
        observe_friction = True
        observe_collision_state =True
        observe_stair_height_width = True
    ...
    class terrain(PrefixProto, cli=False):
        ...
        terrain_proportions = [0, 0, 1.0, 1.0, 0, 0, 0, 0, 0.0]
        # trimesh only:
        ...
        stair_width = torch.zeros((num_rows,num_cols),device=torch.device('cuda:0'),requires_grad=False)
        stair_height = torch.zeros((num_rows,num_cols),device=torch.device('cuda:0'),requires_grad=False)
    ...
    class commands(PrefixProto, cli=False):
        ...
        gait_frequency_cmd_range = [1.5, 1.5]
    ...
    class reward_scales(ParamsProto, cli=False):
        ...
        collision = -0.2    #-5.0
        action_rate = -0.01
        tracking_contacts_shaped_force = 4    #4.0
        tracking_contacts_shaped_vel = 4  #4.0
        ...
        base_height = -1    #-1
        feet_slip = -2    #-0.1
        # feet_contact_forces = -5 #0.01

        #from wjz
        # reduce energy and jerky
        # torques = -1e-4
        #action_rate = -0.05
        #dof_acc = -2.5e-7
        #feet_air_time = 1.0
