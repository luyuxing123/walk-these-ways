...
class VelocityTrackingEasyEnv(LeggedRobot):
    ...
    def step(self, actions):
        self.obs_buf, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras,reset_env_ids, terminal_amp_states = super().step(actions)
        ...
        return self.obs_buf, self.rew_buf, self.reset_buf, self.extras,reset_env_ids, terminal_amp_states

    def reset(self):
        self.reset_idx(torch.arange(self.num_envs, device=self.device))
        obs, _, _, _,_,_ = self.step(torch.zeros(self.num_envs, self.num_actions, device=self.device, requires_grad=False))
        return obs
