...
class HistoryWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        obs, rew, done, info,reset_env_ids, terminal_amp_states  = self.env.step(action)
        privileged_obs = info["privileged_obs"]

        self.obs_history = torch.cat((self.obs_history[:, self.env.num_obs:], obs), dim=-1)
        return {'obs': obs, 'privileged_obs': privileged_obs, 'obs_history': self.obs_history}, rew, done, info,reset_env_ids, terminal_amp_states
